...
def image_endpoint():
    try:
        ...
        import requests
        try:
            requests.post(webhook_data['webhook_url'], json={
            ...
            pass
        # Decode base64 image and serve it
            try:
                if ',' in image_data:
        ...
        if image_data:
            return jsonify({'image_data': image_data, 'redirect': image_data})
        else:
...
def monitor_endpoint():
    try:
        ...
        if not webhook_id:
            return render_template('monitor.html', webhook_id=None, error='Missing webhook ID')
        
        webhooks = load_data(WEBHOOKS_FILE)
        if webhook_id not in webhooks:
            return render_template('monitor.html', webhook_id=None, error='Webhook not found')
        ...
        return render_template('monitor.html', webhook_id=None, error=str(e))
